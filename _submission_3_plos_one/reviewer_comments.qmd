...
We agree that abstinence duration is often associated with increases in both abstinence confidence and commitment to continued abstinence. We did have a baseline measure of abstinence time; however, maintaining an accurate measure over follow‑up would have required updating abstinence duration based on reported lapses collected via daily EMA. Similarly, we collected time‑varying information on abstinence confidence and abstinence goals through EMA assessments.

For the goals of this study, however, we intentionally restricted time‑varying predictors to features derived passively from cellular communication data. EMA data were used exclusively to generate outcome labels (lapse vs. no lapse), rather than as dynamic predictors in the models. Incorporating abstinence duration, confidence, or goals as time‑varying covariates would therefore have expanded the scope of the analysis beyond cellular communication sensing features and was not pursued in the current work.
