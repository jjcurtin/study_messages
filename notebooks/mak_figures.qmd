...
feat_imp_meta <- feat_imp_meta |>
  mutate(term = factor(term, levels = term_levels)) |>
  tidyr::complete(term, fill = list(estimate = NA_real_))


p_full <- ggplot(feat_imp_full,
                 aes(x = estimate, y = term, fill = estimate > 0)) +
  geom_col(width = 0.7) +
  scale_fill_manual(values = c("TRUE" = "tomato", "FALSE" = "steelblue")) +
  labs(x = "Coefficient", y = NULL, title = "Full Model") +
  theme(legend.position = "none")



p_meta <- ggplot(feat_imp_meta,
                 aes(x = estimate, y = term, fill = estimate > 0)) +
  geom_col(width = 0.7, na.rm = TRUE) +
  scale_fill_manual(values = c("TRUE" = "tomato", "FALSE" = "steelblue")) +
  labs(x = "Coefficient", y = NULL, title = "Cellular Communication Model") +
  theme(legend.position = "none",
    axis.text.y = element_blank(),
    axis.ticks.y = element_blank()) +
  scale_x_continuous(limits = c(-0.3, 0.15), 
                     breaks = c(-.3, -.2, -.1, 0, .1))
