...
test_metrics_all <- test_metrics_all |> 
  bind_cols(tibble(`chance model` = rep(0.5, 30))) 
```
...
pp_perf_tibble <- pp_tidy |> 
  ...
  mutate(model = factor(model, levels = c("full model", "baseline model", "metadata model", "chance model"))) |> 
  arrange(model)
...
ci_meta <- pp |>
  contrast_models(list("metadata model"),
                  list("chance model")) |>
  summary(size = 0) |> 
  mutate(contrast = "metadata model vs. chance model")
...
ci_meta <- ci_meta |>
  left_join(ci_median_meta, by = c("contrast"))

ci_meta |>
  write_csv(here::here(path_models, "contrast_meta.csv"))
