...
```{r}
if (file.exists("objects/pp_auroc.rds")){
  pp <- read_rds("objects/pp_auroc.rds")
} else{
  pp <- auroc |> 
  ...
  pp |> 
    write_rds("objects/pp_auroc.rds")
}
...
pp_tidy |> 
  ...
  geom_density(aes(x = posterior), color = c("#80B1D3"), 
               fill = c("#B3CFE8"), linewidth = 1.2, alpha = .6) +
  # geom_segment(mapping = aes(y = y, yend = y, x = lower, xend = upper), color = c("#80B1D3"),
  #              data = ci) +
  geom_errorbar(data = ci, aes(y = y, xmin = lower, xmax = upper), 
                linewidth = .8, width = 2, color = c("#4292C6")) +
  geom_vline(xintercept = median(pp_tidy$posterior), linetype = "dashed", color = "red") +
