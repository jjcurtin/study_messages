...
data <- read_csv(here::here(path_shared, "features_meta_day_24h_v6.csv"),
  ...
  select(-c(demo_age:demo_race, days_abstinent:stress_total, 
            day_start, day_end))
```
...
rec <- recipe(y ~ ., data = data) |> 
  step_rm(subid, dttm_label, strat, label_num) |> 
...
of 323 features, 159 were retained
```{r}
tidy(fit_best) |> 
