...
For each individual lapse window, we have predictor sets that differ in prediction window length and their message type. We define text prediction windows to be 1-day, 2-day, 3-day, and 1-week preceding the lapse window. Within each prediction window, we will use two configuration methods to construct feature sets (referred to as "document" in the following sections). First, we will combine both incoming and outgoing messages in one document as predictor. Second, we will separate incoming messages and outgoing messages into two documents as predictors.

## Feature Engineering

Text messages will serve as the only raw source for all feature engineering. The document sets will go through a generic pre-processing step that involves transformation of emojis to be readable text, tokenization into words and removal of punctuations and numbers. The documents from each of the configurations will go through the natural language processing techniques described below independently. 

We will use several natural language processing techniques to transform documents into machine-interpretable data and input them into machine learning algorithms. The methods we consider include: 1) topic modeling; 2) sentiment analysis; 3) n-gram approach; 4) dictionary approaches (LIWC and customized alcohol-relevant dictionaries); and 5) word embeddings. Each feature engineering technique will be evaluated separately. 
...
Topic modeling is an unsupervised machine learning method to identify clusters of topics in a body of text. We will make all words lower-case and remove stop words (i.e., common words without significant meaning). We will also perform stemming to transform the words into their root form. We will further apply the cleaned tokens into one of the most popular topic modeling technique -- Latent Dirichlet Analysis (LDA). LDA unravels the hidden structure in the relationships between words in a document and groups similar words into topics. It assumes that different documents (i.e., SMS messages) have same topic clusters but with different distribution. As such, different topic clusters might have common words.

### Sentiment Analysis

Research has found that positive/negative emotional state to be associated with increased alcohol relapse risks [@mckayConceptualMethodologicalAnalytical2006b; @mckayStudiesFactorsRelapse1999a; @brownCharacteristicsRelapseFollowing1989]. Sentiment Analysis infers the emotional underpinning of texts. We will extract both emotional valence (i.e., positive, neutral, and negative emotional tone) and eight basic emotion categories (e.g., anger, fear, anticipation, trust, surprise, sadness, joy, and disgust) from SMS messages. We will make all words in lower-case, and perform stemming and removal of stop words on our feature sets. We will then assign each document a score in those ten categories by summing sentiment content of all tokens in that message.

### N-grams Approach

A bag-of-words approach represents word occurrences inside a document. Unlike other feature engineering methods, we will tokenize documents into one-gram, two-gram and one-gram and two-grams combined tokens in the generic pre-processing step. Then we will make all text in lower-case, remove stop words and stem the n-grams. We will then analyze these n-grams based on their term frequency (tf), and a combination of term frequency and inverse document frequency (tf-idf).
