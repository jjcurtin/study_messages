...
For each individual lapse window, we have twelve predictor sets that differ in prediction window length and their message type. We define text prediction windows to be 1-hour, 2-hour, 3-hour, and 1-week preceding the lapse window. Within each prediction window, we will filter the messages to be incoming only, outgoing only, and incoming and outgoing combined. We will then combine messages together for each of the filtering method. Each of the final twelve predictor sets will be referred to as a "document" in the following sections. 

## Feature Engineering

Text messages will serve as the only raw source for all feature engineering. The document sets will go through a generic pre-processing step that involves transformation of emojis to be readable text, tokenization into words and removal of punctuations and numbers. The derived feature sets from each of the twelve documents will go through the natural language processing techniques described below independently. 
