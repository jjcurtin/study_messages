...
<!--I am somewhat uncomfortable with suggesting DTx are full blown treatments.  Better to think of them as providing continuing care subequent to treatment-->
...
<!--it did extract signal.  The auROC was signficantly > .5).  But it did not extract sufficient signal to be used in any meaningful way.  Keep the distiction between signficant and clnically important clear.-->
...
<!--second point.  New paragraph.   Just a 2-3 sentences-->

Our LIWC model only serves as a baseline model, and more robust features need to be engineered from SMS messages to achieve better prediction. To explore more robust features, we might explore different model configurations (e.g., varying in prediction window length, message type) and other feature NLP methods (e.g., topic modeling, n-grams).

<!--I thought you already considered separate features for incoming and outgoing?-->

<!--CY: We didn't distinguish between the two in this study. Our configurations only differ in normalziation and prediction window length.-->

<!--good point that doing separate in/out may cause more problems with missing data.   Maybe you will need to only use the past week window?-->
...
<!--might also comment on the utility of text vs. other raw sensed data given ease of access. text is less burdensome than EMA, good. But EMA appears to be a much stronger signal. Gps is not burdensome either and more promising out of the box (with little feature engineering to date we got more signal). And both EMA and gps are easily available across platforms but text is not currently available on iOS. This diminishes further interest in text-->

Results in this study indicate that text message might not be potentially valuable raw data source in predicting alcohol lapses in AUD patients. Although SMS messages offer the benefits of minimal burden on users, this advantage cannot compensate for their low prediction precision. EMA measures, despite their somewhat higher burden, are capable of capturing robust signals. GPS sensing, which is also not burdensome, can obtain some signals with little feature engineering. Note that both EMA and GPS sensing are easily accessible across platforms. Nonetheless, as SMS is not currently available in ios platform, we might have diminished further interest in this data source.
